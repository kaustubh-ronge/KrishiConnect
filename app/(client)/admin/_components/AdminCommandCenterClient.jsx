...
export default function AdminCommandCenterClient({
   ...
   useEffect(() => {
      if (!mounted) return;
      if (['farmers', 'agents', 'delivery', 'catalog'].includes(activeView)) {
   ...
   const fetchDirectoryData = async (view) => {
      // Don't refetch if we already have data (optional, but good for performance)
      setIsLoading(true);
      try {
         if (view === 'farmers') {
   ...
   const handleBulkApprove = async () => {
      ...
      try {
         ...
         setPendingProfiles(previousProfiles);
         setSelectedIds(selectedIds); // Optional: restore selection
   ...
   const openOrderAudit = async (orderId) => {
      ...
      try {
         ...
         // Fetch full seller bank details for this order
         const bankRes = await viewBankAction(orderId);
         
         // Map sellers correctly for the modal
         const uniqueSellers = [];
         const seenSellers = new Set();
         
         // Use data from bankRes if successful, otherwise fallback to order items
         ...
         <main className="flex-grow flex flex-col min-w-0 bg-slate-50">
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar">
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar">
                           <Table>
                              ...
                              <TableRow>
                                 ...
                                 <TableHead className={activeView === 'verifications' ? "pl-2" : "pl-8"}>{activeView === 'orders' || activeView === 'disputes' ? 'ORDER ID & BUYER' : 'IDENTITY & NAME'}</TableHead>
                                 <TableHead>{activeView === 'orders' || activeView === 'disputes' ? 'PAYMENT' : 'LOCATION & DATA'}</TableHead>
                                 <TableHead>{activeView === 'orders' || activeView === 'disputes' ? 'ORDER STATUS' : 'JOIN DATE'}</TableHead>
                                 <TableHead>{activeView === 'orders' || activeView === 'disputes' ? 'METHOD' : 'ACCOUNT STATE'}</TableHead>
                                    ...
                                    <TableCell className={activeView === 'verifications' ? "pl-2" : "pl-8"}>
                                       <div className="flex items-center gap-4">
                                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shadow-sm border ${
                                             activeView === 'catalog' ? 'bg-purple-50 text-purple-600 border-purple-100' :
                                       ...
                                       <div className="flex items-center gap-3">
                                          <div className="flex flex-col">
                                              {activeView === 'orders' || activeView === 'disputes' ? (
                                                 ...
                                                 <>
                                                    <span className="text-[10px] font-black text-slate-900 leading-none">{s(item.city || item.category || item.vehicleType || (item.items?.[0]?.productName + '...'))}</span>
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">{s(item.district || (item.shippingAddress ? item.shippingAddress.slice(0, 20) + '...' : ''))}</span>
                                                 </>
                                              )}
                                          </div>
                                       </div>
               ...
               {activeView === 'finance' && (
                ...
                {/* PROFILE MODAL - ENHANCED FOR REAL-LIFE TEAMS */}
            <Dialog open={isProfileModalOpen} onOpenChange={setIsProfileModalOpen}>
               <DialogContent className="sm:max-w-2xl p-0 border-0 bg-white shadow-2xl rounded-[2rem] overflow-hidden max-h-[85vh] flex flex-col custom-scrollbar">
                  <div className={`p-5 text-white shrink-0 ${selectedProfile?.role === 'farmer' ? 'bg-emerald-600' : 'bg-indigo-600'}`}>
                     <div className="flex justify-between items-start relative z-10">
                        <div className="space-y-2">
                           ...
                           <DialogTitle className="text-xl font-black tracking-tighter leading-none">{s(selectedProfile?.displayName)}</DialogTitle>
                           <div className="flex items-center gap-4 text-white/70 font-bold text-[11px]"><Mail className="h-3.5 w-3.5" /> {selectedProfile?.user?.email} | <Phone className="h-3.5 w-3.5" /> {s(selectedProfile?.phone)}</div>
                        </div>
                        <div className="w-12 h-12 bg-white/10 rounded-xl border border-white/20 flex items-center justify-center text-lg font-black">{selectedProfile?.displayName?.[0]}</div>
                     </div>
                  </div>
                  <Tabs defaultValue="identity" className="flex-grow flex flex-col overflow-hidden">
                     <div className="px-6 bg-slate-50 border-b border-slate-200 shrink-0">
                        <TabsList className="bg-transparent h-10 gap-6">
                           <TabsTrigger value="identity" className="h-full bg-transparent border-0 font-black text-[9px] uppercase tracking-widest data-[state=active]:border-b-[2px] data-[state=active]:border-indigo-600 rounded-none shadow-none text-slate-400 data-[state=active]:text-indigo-600">Member Info</TabsTrigger>
                           <TabsTrigger value="documents" className="h-full bg-transparent border-0 font-black text-[9px] uppercase tracking-widest data-[state=active]:border-b-[2px] data-[state=active]:border-indigo-600 rounded-none shadow-none text-slate-400 data-[state=active]:text-indigo-600">Documents</TabsTrigger>
                           <TabsTrigger value="admin" className="h-full bg-transparent border-0 font-black text-[9px] uppercase tracking-widest data-[state=active]:border-b-[2px] data-[state=active]:border-rose-600 rounded-none shadow-none text-slate-400 data-[state=active]:text-rose-600">Admin Notes</TabsTrigger>
                        </TabsList>
                     </div>
  
                     <div className="flex-grow overflow-y-auto custom-scrollbar bg-white">
                        <div className="p-5">
                           <TabsContent value="identity" className="m-0 space-y-8 animate-in fade-in duration-300 pr-2">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                       <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-slate-900 font-bold text-sm shadow-inner">{s(selectedProfile?.address)}, {s(selectedProfile?.city)}, {s(selectedProfile?.district)}</div>
                                       {(selectedProfile?.lat && selectedProfile?.lng) && <Button variant="outline" className="h-10 w-full rounded-xl border-slate-200 font-black text-rose-600 text-[10px] gap-2 uppercase" onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${selectedProfile.lat},${selectedProfile.lng}`)}><LucideMap className="h-4 w-4" /> Open Maps</Button>}
                                    </div>
                                 </div>
                                 <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 font-black text-[9px] text-slate-900">
                                       <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col gap-0.5"><span className="text-[8px] text-slate-400 uppercase tracking-widest">Aadhar UID</span><span className="text-base tracking-widest font-mono uppercase">{s(selectedProfile?.aadharNumber)}</span></div>
                                       <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 text-indigo-900 text-base font-black uppercase">Role: {selectedProfile?.role}</div>
                                    </div>
                                 </div>
                              </div>
                           </TabsContent>
 
                           <TabsContent value="documents" className="m-0 space-y-10 animate-in fade-in duration-300 pr-2">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 {['aadharFront', 'aadharBack', 'licenseImage'].map((field, i) => (
                                    selectedProfile?.[field] && (
                                       <div key={i} className="space-y-3">
                                          ...
                                          <div className="aspect-[1.6/1] bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 overflow-hidden group relative">
                                             ...
                                             <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all duration-300 backdrop-blur-sm">
                                                <Button variant="secondary" className="h-10 px-8 rounded-xl font-black text-xs" onClick={() => window.open(selectedProfile[field])}>View Full</Button>
                                             </div>
                                          </div>
                                       </div>
                                    )
                           ...
                           <TabsContent value="admin" className="m-0 space-y-6 animate-in fade-in duration-300 pr-2">
                              <h5 className="flex items-center gap-4 text-xs font-black text-slate-400 uppercase tracking-widest"><StickyNote className="h-6 w-6 text-rose-500" /> Internal Notes</h5>
                              <Textarea placeholder="Type internal justification here..." className="h-40 rounded-2xl border-slate-200 p-6 font-bold text-slate-700 bg-slate-50 shadow-inner" value={adminNote} onChange={e => setAdminNote(e.target.value)} />
                              <p className="text-[9px] font-black text-slate-400 uppercase text-center">Visible to admin team only.</p>
                           </TabsContent>
                        </div>
                     </div>
                  </Tabs>

                  <DialogFooter className="p-4 bg-slate-50 border-t border-slate-200 flex gap-4 shrink-0">
                     <Button variant="outline" className="h-10 px-6 rounded-xl font-black text-rose-600 border-rose-200 hover:bg-rose-50 text-[10px] uppercase tracking-widest transition-all" onClick={() => { setIsProfileModalOpen(false); handleReject(selectedProfile?.userId, selectedProfile?.role, selectedProfile?.displayName); }}>Reject</Button>
                     <Button className="flex-grow h-10 bg-emerald-600 text-white rounded-xl font-black text-xs shadow-lg shadow-emerald-500/20 uppercase tracking-widest" onClick={() => { setIsProfileModalOpen(false); handleApprove(selectedProfile?.userId, selectedProfile?.role, selectedProfile?.displayName); }}>Verify Member</Button>
                  </DialogFooter>
               </DialogContent>
            </Dialog>

            {/* ORDER AUDIT MODAL - FINANCIAL HARDENING */}
            <Dialog open={isOrderModalOpen} onOpenChange={setIsOrderModalOpen}>
               <DialogContent className="sm:max-w-4xl p-0 border-0 bg-white shadow-2xl rounded-[2rem] overflow-hidden max-h-[85vh] flex flex-col custom-scrollbar">
                  <div className="bg-slate-950 p-5 text-white shrink-0">
                     <div className="flex items-center justify-between">
                        <div>
                           <Badge className="bg-indigo-500/10 text-indigo-400 border-0 text-[8px] font-black uppercase px-4 py-1 rounded-full mb-2 tracking-widest shadow-inner">Sales Audit</Badge>
                           <DialogTitle className="text-xl font-black tracking-tighter leading-none mb-1">Order: #{selectedOrder?.id?.slice(-8).toUpperCase()}</DialogTitle>
                           <div className="flex items-center gap-4 mt-2"><Badge className="bg-indigo-600 text-white border-0 text-[8px] px-3 py-1 uppercase font-black rounded-full tracking-widest">{getFriendlyStatus(selectedOrder?.orderStatus)}</Badge></div>
                        </div>
                        <div className="text-right">
                           <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Total Bill</p>
                           <p className="text-xl font-black text-white tracking-tighter">{"\u20B9"}{sNum(selectedOrder?.totalAmount).toLocaleString()}</p>
                        </div>
                     </div>
                  </div>
                        ...
                        {isLoadingDetails ? <div className="py-40 text-center text-xs font-black text-slate-300 uppercase tracking-[0.5em] animate-pulse">Running Ledger Audit...</div> : selectedOrder && (
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                              <div className="space-y-10">
                                 ...
                                 <Card className="rounded-3xl border-0 bg-white p-6 space-y-6 shadow-xl shadow-slate-200/50">
                                    <div className="flex justify-between items-center"><span className="text-[10px] font-black text-slate-400 uppercase">Seller Payment</span><span className="text-lg font-black text-slate-900">{"\u20B9"}{sNum(selectedOrder.sellerAmount)}</span></div>
                                    <div className="flex justify-between items-center"><span className="text-[10px] font-black text-slate-400 uppercase">Delivery Fee</span><span className="text-lg font-black text-slate-900">{"\u20B9"}{sNum(selectedOrder.deliveryFee)}</span></div>
                                    <div className="flex justify-between items-center"><span className="text-[10px] font-black text-slate-400 uppercase">Platform Profit</span><span className="text-lg font-black text-indigo-600">{"\u20B9"}{sNum(selectedOrder.platformFee)}</span></div>
                                    <div className="pt-6 border-t border-slate-100 flex justify-between items-center"><span className="text-[10px] font-black text-slate-900 uppercase">Verified Ledger</span><Badge className="bg-emerald-500 text-white border-0 text-[9px] px-3 py-1 font-black uppercase rounded-lg">100% Correct</Badge></div>
                                 </Card>
                                 ...
                                 <div className="space-y-4">
                                    {selectedOrder.items?.map((it, idx) => (
                                       <div key={idx} className="flex items-center justify-between p-6 bg-white border border-slate-100 rounded-3xl shadow-sm">
                                          <div className="flex items-center gap-6">
                                             <div className="w-16 h-16 bg-slate-50 rounded-2xl overflow-hidden border border-slate-100 flex items-center justify-center">
                                                {it.image ? <img src={it.image} alt={it.productName} className="w-full h-full object-cover" /> : <ImageIcon className="h-6 w-6 text-slate-300" />}
                                             </div>
                                             <div className="flex flex-col"><span className="text-lg font-black text-slate-900">{it.productName}</span><span className="text-[10px] font-black text-slate-400 uppercase">{it.quantity} {it.unit} sold</span></div>
                                          </div>
                                       </div>
                                    ))}
                                 </div>
                              ...
                              <div className="space-y-10">
                                 ...
                                 <div className="space-y-6">
                                    {selectedOrder.sellers?.map((sObj, sIdx) => (
                                       <Card key={sIdx} className="rounded-[2.5rem] border-0 bg-slate-950 text-white p-8 shadow-2xl relative overflow-hidden">
                                          <div className="relative z-10 space-y-6">
                                             <div className="flex justify-between items-start">
                                                <div>
                                                   <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Target Seller</p>
                                                   <p className="text-sm font-black text-white">{sObj.name || 'Unknown Seller'}</p>
                                                </div>
                                                <div className="text-right">
                                                   <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Payout Amount</p>
                                                   <p className="text-lg font-black text-white">₹{sObj.totalEarned?.toLocaleString() || '0'}</p>
                                                </div>
                                             </div>
                                             {!sObj.bankDetails?.accountNumber ? (
                                                <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center gap-3">
                                                  ...
                                                  <div>
                                                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest leading-none">Action Required</p>
                                                    <p className="text-[9px] text-rose-400 font-bold mt-1">Seller has not provided bank details. Payout cannot be processed.</p>
                                                  </div>
                                                  <Button 
                                                    size="sm" 
                                                    variant="secondary" 
                                                    className="ml-auto h-8 rounded-lg text-[9px] font-black uppercase"
                                                    onClick={() => window.open(`tel:${sObj.phone}`)}
                                                  >
                                                    <Phone className="h-3 w-3 mr-2" /> Call Seller
                                                  </Button>
                                                </div>
                                              ) : (
                                                <div>
                                                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Bank Account Node</p>
                                                  <p className="text-xl font-mono font-black text-white tracking-widest">{sObj.bankDetails.accountNumber}</p>
                                                </div>
                                              )}
                                             <div className="grid grid-cols-2 gap-6 border-t border-white/5 pt-6">
                                                <div><p className="text-[9px] text-slate-500 uppercase">Bank Name</p><p className="text-xs font-black text-indigo-400 uppercase">{sObj.bankDetails?.bankName || 'NOT SET'}</p></div>
                                                <div><p className="text-[9px] text-slate-500 uppercase">IFSC Code</p><p className="text-xs font-black text-emerald-400 uppercase font-mono">{sObj.bankDetails?.ifscCode || 'NOT SET'}</p></div>
                                             </div>
                                             <div className="pt-4 border-t border-white/5">
                                                <p className="text-[9px] text-slate-500 uppercase mb-1">UPI Information</p>
                                                <p className="text-xs font-black text-slate-300">{sObj.bankDetails?.upiId || 'No UPI ID'}</p>
                                             </div>
                                          </div>
                                       </Card>
                                    ))}
                                    {(!selectedOrder.sellers || selectedOrder.sellers.length === 0) && (
                                       <div className="p-10 text-center bg-slate-100 rounded-[2.5rem] border-2 border-dashed border-slate-200">
                                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No Seller Nodes Identified</p>
                                       </div>
                                    )}
                                 </div>

                                 <h5 className="flex items-center gap-4 text-xs font-black text-slate-400 uppercase tracking-widest"><Truck className="h-7 w-7 text-indigo-400" /> Courier Node</h5>
                                 {selectedOrder.deliveryPartners?.map((dp, idx) => (
                                    <div key={idx} className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex items-center justify-between">
                                       <div className="flex items-center gap-4">
                                          <div className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 font-black">DP</div>
                                          <div className="flex flex-col"><span className="text-base font-black text-slate-900">{dp.partnerName}</span><span className="text-[10px] font-bold text-slate-400 uppercase">Logistics Member</span></div>
                                       </div>
                                       <div className="text-right"><span className="text-xl font-black text-slate-900">{"\u20B9"}{dp.totalPrice}</span></div>
                                    </div>
                                 ))}
                              </div>
                           </div>
                  ...
                  <DialogFooter className="p-4 bg-white border-t border-slate-200 flex gap-4 shrink-0">
                     <Button variant="ghost" className="font-black text-[10px] text-slate-400 h-10 px-8 rounded-xl uppercase tracking-widest" onClick={() => setIsOrderModalOpen(false)}>Close</Button>
                     <Button className="flex-grow h-10 bg-emerald-600 text-white rounded-xl font-black text-xs shadow-lg shadow-emerald-600/30 uppercase tracking-widest" onClick={() => { setIsOrderModalOpen(false); handleSettle(selectedOrder.id); }}>Settle Payout</Button>
                  </DialogFooter>
               </DialogContent>
            </Dialog>
            </div>
