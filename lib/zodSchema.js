...
export const farmerSchema = z.object({
  ...
  upiId: z.string().min(3, "Please enter a valid UPI ID or Transaction ID").max(256).optional().or(z.literal("")),
  ...
  if (data.usagePurpose === "buy_and_sell") {
    ...
    if (!data.upiId || data.upiId === "") {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "UPI ID or Transaction ID is required for receiving payments", path: ["upiId"] });
    }
...
export const deliverySchema = z.object({
  ...
  upiId: z.string().min(3, "Please enter a valid UPI ID or Transaction ID").max(256),
  paymentType: z.enum(['UPI', 'TRANSACTION']).optional().or(z.literal('')),
